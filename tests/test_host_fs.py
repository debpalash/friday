...
class PrivateFileTests(unittest.TestCase):
    ...
    def test_posix_identity_checks(self) -> None:
        ...
        loose_metadata = os.stat_result((0o100640, 0, 0, 1, os.geteuid(), 0, 1, 0, 0, 0))
        self.assertTrue(fs.is_private_regular(private.lstat()))
        self.assertFalse(fs.is_private_regular(loose_metadata))
        self.assertTrue(fs.owned_by_caller(private.lstat()))
        self.assertTrue(fs.private_mode_ok(private.lstat()))
        self.assertFalse(fs.private_mode_ok(loose_metadata))
        self.assertTrue(fs.private_mode_ok(loose_metadata, mask=0o022))
        ...
        self.assertTrue(fs.is_private_regular(loose.lstat()))
