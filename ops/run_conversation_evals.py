...
def _runtime() -> tuple[str, str, str]:
    ...
    if (parsed.scheme != "http" or parsed.hostname != "127.0.0.1"
        ...
        raise RuntimeError(
            "conversation evaluation requires exact loopback model authority")
...
def main() -> int:
    base_url, model, fingerprint = _runtime()
    client = OpenAI(
    ...
    result = ConversationQualityEvalRunner(
        complete, model=model, runtime_fingerprint=fingerprint).run(
