#!/usr/bin/env python3
"""Run the separate exact-graded document/OCR semantic scorecard."""
...
def _runtime() -> tuple[str, str, str]:
    ...
    try:
        ...
        with os.fdopen(descriptor, "rb") as stream:
            ...
            if not stat.S_ISREG(metadata.st_mode) or not 2 <= metadata.st_size <= 128_000:
                raise RuntimeError("resolved runtime manifest is not a bounded regular file")
    ...
    if (parsed.scheme != "http" or parsed.hostname != "127.0.0.1"
        ...
        raise RuntimeError("grounded evaluation requires exact loopback model authority")
    return base_url.rstrip("/"), model, fingerprint
...
def main() -> int:
    base_url, model, runtime_fingerprint = _runtime()
    client = OpenAI(
    ...
    result = GroundedQAEvalRunner(
        ...
        model=model, runtime_fingerprint=runtime_fingerprint).run(
            REPO / "evals" / "document-reasoning-v1.json")
